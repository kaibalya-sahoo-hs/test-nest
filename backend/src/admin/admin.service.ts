...
export class AdminService {
  ...
  async createUsers(
    users: [{ name: string; email: string; role: 'string'; password: string }],
  ) {
    try {
      console.log(users);
    ...
    try {
      ...
      if (!allowed.includes(status)) {
        throw new BadRequestException(
          `Cannot transition from '${vendor.vendorStatus}' to '${status}'`,
        );
      }
  ...
  async getPayments(orderId) {
    const order = await this.orderRepo.findOne({
      where: { id: orderId },
      relations: ['payments', 'payments.statusHisotry'],
    });
