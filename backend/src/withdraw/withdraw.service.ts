...
export class WithdrawService {
    ...
    async createWithdrawal(userId, role, amount = 0) {
        let user;
        if (role === "vendor") {
            user = await this.vendorRepo.findOne({where: {id: userId}})
        }else{
        ...
        let withdrawal;
        if(role === "vendor"){
             await this.vendorRepo.save(user);
    ...
    async getWithdrawHistory(userId, role) {
        const withdraws = await this.withdrawRepo.find({ where: role === "vendor" ? { vendor: { id: userId } } : {user: {id:userId}}})
