...
function AdminOrdersPage() {
    ...
    if (loading) return <div className="p-10 text-center font-sans">Loading Orders...</div>;
                            ...
                            {orders.map((order) => (
                                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        {order.items.map((item) => (
                                            <div key={item.id} className="flex items-center gap-3 mb-2 last:mb-0">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-gray-500">Qty: {item.quantity} × ₹{item.product.price}</div>
                                                </div>
                                            </div>
                                        ))}
                                    </td>
                                    ...
                                    <td className="p-4 align-top">
                                        ...
                                        {order.discount > 0 && (
                                            <div className="text-xs text-green-600">Discount: -₹{order.discount}</div>
                                        )}
                                    ...
                                    {/* Payment Logs Column */}
                                    <td className="p-4 align-top">
                                        <div className="space-y-2 py-2 px-4 rounded-xl bg-blue-500 text-white w-fit" onClick={() => navigate(`/admin/orders/${order.id}`)}>
                                            View Payment Logs
                                        </div>
                                    </td>
