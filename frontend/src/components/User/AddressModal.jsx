      ...
      <div role="dialog" aria-labelledby="address-modal-title" data-testid="address-modal" className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <form onSubmit={handleSubmit} className="bg-white w-full max-w-lg rounded-2xl shadow-2xl p-6 overflow-y-auto max-h-[90vh]">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <input required placeholder="Phone Number" className="w-full p-3 border rounded-lg" value={formData.phoneNumber} onChange={e => setFormData({...formData, phoneNumber: e.target.value})} />
            <textarea required placeholder="Street Address" className="w-full p-3 border rounded-lg" value={formData.streetAddress} onChange={e => setFormData({...formData, streetAddress: e.target.value})} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <input required placeholder="State" className="p-3 border rounded-lg" value={formData.state} onChange={e => setFormData({...formData, state: e.target.value})} />
            </div>
  
            <div className="flex gap-4 items-center mt-2">
              <label className="text-sm font-bold text-gray-700">Type:</label>
              {['home', 'work', 'other'].map(t => (
                <label key={t} className="flex items-center gap-1 text-sm cursor-pointer capitalize">
                  <input type="radio" checked={formData.addressType === t} onChange={() => setFormData({...formData, addressType: t})} /> {t}
                </label>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3 mt-8">
            ...
            <button type="submit" className="bg-blue-600 text-white px-8 py-2 rounded-lg font-bold shadow-lg shadow-blue-200">Save Address</button>
          </div>
        </form>
      </div>
    );
